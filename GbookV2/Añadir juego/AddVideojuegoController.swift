...
class AddVideojuegoController : UIViewController , UIPickerViewDelegate , UIPickerViewDataSource,UINavigationControllerDelegate{
    ...
    func assignbackground(){
        ...
        backgroundImage.image = UIImage(named: "prueba")
    ...
    func borderColor(){
        ...
        buttonAddPhoto.layer.shadowColor = UIColor.black.cgColor
        buttonAddPhoto.layer.shadowOffset = CGSize(width: 5, height: 5)
        buttonAddPhoto.layer.shadowRadius = 5
        buttonAddPhoto.layer.shadowOpacity = 1.0
        buttonPost.layer.shadowColor = UIColor.black.cgColor
        buttonPost.layer.shadowOffset = CGSize(width: 5, height: 5)
        buttonPost.layer.shadowRadius = 5
        buttonPost.layer.shadowOpacity = 1.0
        
        buttonAddPhoto.layer.cornerRadius = 12
    ...
    @IBAction func Add(_ sender: UIButton) {
        ...
        if(textTitulo.text?.isEmpty == true){
            showToast.mostrarToastError(controller: self, message: "Tienes que introducir un titulo", seconds: 2)
        }
        else{
        self.saveFIRData()
...
extension AddVideojuegoController {
    ...
    func saveImage(profileURL:URL, completion: @escaping ((_ url: URL?) -> ())){
        ...
        if opinion == true{
            ...
            db.collection("Videojuego").addDocument(data: ["Nombre" : textTitulo.text!, "Check" : "Opinion" , "FotoVideojuego": profileURL.absoluteString] as [String:Any])
            db.collection("Post").addDocument(data: ["Nombre" : textTitulo.text! , "Check" : "Opinion" ,"Email" : email!,"ImagenAvatar" : imagenVideojuego , "NombredeUsuario" : userName,"Likes" : "0", "CheckLike\(nuevoEmail!)" : false,"CheckLikeprueba2@gmailcom" : false,"CheckLikejesus@gmail.com" : false ,"FotoVideojuego": profileURL.absoluteString] as [String:Any])
            myGroup.leave()
        
    }else {
            db.collection("Videojuego").addDocument(data: ["Nombre" : textTitulo.text! , "Check" : "Compra","FotoVideojuego": profileURL.absoluteString] as [String:Any])
            db.collection("Post").addDocument(data: ["Nombre" : textTitulo.text! , "Check" : "Compra" ,"Email" : email!,"ImagenAvatar" : imagenVideojuego , "NombredeUsuario" : userName,"Likes" : "0","CheckLike\(nuevoEmail!)" : false,"CheckLikeprueba2@gmailcom" : false,"CheckLikejesus@gmail.com" : false ,"FotoVideojuego": profileURL.absoluteString] as [String:Any])
            myGroup.leave()
    }
